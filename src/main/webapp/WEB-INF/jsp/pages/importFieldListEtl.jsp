...
<script language="javascript">
    ...
    function doUpload() {
        var etls = $('#fieldTble').datagrid('getChecked');
        var mydate=JSON.stringify(etls);
//        var mydata=JSON.stringify(etls);
        $.ajax({
            url: '${pageContext.request.contextPath}/uploadFieldExcel/fileuploadEtl' ,
            type: 'post',
            dataType :'json',
            data:{mydate:mydate},
            async: false,
            success: function (data) {
                console.log("选着的数据成功导入");
                if(data.success){
    ...
    function checkState() {
        $.get("${pageContext.request.contextPath}/uploadFieldExcel/fileUploadState",function (data) {
            if(data.success){
                $("#s_id").text(data.s+'条');
                $("#f_id").text(data.f+'条');
                if(data.ipmState){
