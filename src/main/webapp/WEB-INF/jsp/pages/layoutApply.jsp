        ...
        <form id="addForm" >
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0" class="table-new">
                <tr>
                    ...
                    <td>
                        <label>
                            <input type="text" onblur="validateXSS(this)" id="layout_no"  name="layout_no" class="easyui-validatebox" required placeholder="请输入建设工程规划许可证号" />
                        </label>
                    </td>
                </tr>
                <tr>
            ...
            <div class="apply-btn">
                ...
                <input type="button" class="yellow" id="saveApply" value="保 存" />
                <input type="button" class="yellow" id="saveApplyFile"value="提 交" />
            </div>
        </form>
    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("#addContract").on("click", function(){
            openDialog("width:900,height:400,winId:'contractList',title:'新增',url: '${pageContext.request.contextPath }/layout/addContractNo',buttons:[{text: '确认',handler:function(){addContract()}}]");
        });

        //增加不动产证书号
        $("body").on("click",".addNo",function(){
            openDialog("width:900,height:400,winId:'addNumDialog',title:'新增',url: '${pageContext.request.contextPath }/layout/addTypeNo',buttons:[{text: '确认',handler:function(){addTypeNo()}}]");
        });
        ...
        $("#saveApply").on("click",function(){
            ...
            if(!$("#contractAll").val()){
                $.messager.alert('提示','请添加土地合同编号','info');
                return;
            };
            ...
            if(! $('#print_date').datebox('getValue')){
                $.messager.alert('提示','请填写发证日期','info');
                return;
            }

            var flag = $("#addForm").form('validate');
            if(flag){
                $("input[name = file_status]").val('0');
                var paramObj = {};
