...
<div id="toolbar">
        ...
        <form id="lForm">
            ...
            <label>
                <select name="searchType">
                    ...
                    <option value="layout_no"  >建设工程规划许可证号</option>
                    <option value="construction_project_name">项目名称</option>
                    <option value="address">项目地址</option>
                    <option value="construction_unit">建设单位</option>
                </select>
                <input type="text" name="searchValue"  placeholder="请输入查询参数值" />
            </label>
            <label>
                <select name="timeType" >
                    <option value="print_date">发证时间</option>
    ...
    <div class="table-toolbar">
        <button id="addFill" data-url="${pageContext.request.contextPath}/layout/apply?type=" data-type="add">新增</button>
        <button id="importLayout">导入</button>
...
<table id="fieldTble" class="easyui-datagrid" data-options="
    ...
    <tr>
        ...
        <th data-options="field:'region',width:40">区域</th>
        <th data-options="field:'layout_no',width:50">建设工程规划许可证号</th>
        <th data-options="field:'construction_project_name',width:50">项目名称</th>
        <th data-options="field:'address',width:50">项目地址</th>
        <th data-options="field:'construction_scale',width:50">规划总建筑面积（㎡）</th>
        <th data-options="field:'construction_unit',width:50">建设单位</th>
        <th data-options="field:'create_time',width:40">录入时间</th>
        <th data-options="field:'print_date',width:40">发证日期</th>
        <th data-options="field:'file_status',width:40">状态</th>
        <th data-options="field:'true',align:'center',formatter:rowFormater,width:40">操作</th>
    </tr>
    </thead>
    <tbody>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("#deleteFill").on("click",function(){
            ...
            $.get("${pageContext.request.contextPath}/layout/deleteField",{ids:ids},function (data) {
                if(data.success){
                    var a = rows.length;
                    var b = data.result;
                    var c = a-b;
    ...
    function search() {
        var paramObj = {};
        var paramArr = $("#lForm").serializeArray();
        ...
        opts.url="${pageContext.request.contextPath}/layout/queryLayout";
    ...
    //导入
    $("#importLayout").on("click",function(){
        openDialog("title:'批量导入',winId:'importLayoutDialog',url:'${pageContext.request.contextPath}/uploadLayoutExcel/importLayout',buttons:[{text:'下一步',handler:function(){importLayoutNext()}}],width:570,height:205");
    });

    //导入下一步
    function importLayoutNext(){
        var file = $("#importLayoutFile").val();
        if(!file){
        ...
        openDialog("title:'批量导入',winId:'importLayoutList',url:'${pageContext.request.contextPath}/uploadLayoutExcel/importLayoutList',buttons:'none',width:670,height:445");
        $("#importLayoutList").dialog({
            id:'importLayoutList',
            closable: false,
            buttons:[{
                text:'完成',
    ...
    //导入临时表转入正式表
    function transferTable4Layout(){
        $.get("${pageContext.request.contextPath}/uploadLayoutExcel/transferTable",function (data) {
            if(data.success){
