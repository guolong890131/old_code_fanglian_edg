...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $("body").on("click",".addEstate",function(){
            ...
            if($("#addEstateNo").val().length<5){
                $.messager.alert('提示','请输入正确的土地合同编号','info');
                return;
            }
            ...
            $.each($("#estateAll").val().substring(0,$("#estateAll").val().length-1).split(','), function(key, value){
        ...
        $("body").on("click",".addContract",function(){
            ...
            console.debug($("#contractAll").val());
            $.each($("#contractAll").val().substring(0,$("#contractAll").val().length-1).split(','), function(key, value){
                spanHtml += '<span data-no="'+ value +'">'+ value +'<b class="del">x</b></span>';
            });
        ...
        $("#saveApply").on("click",function(){
            ...
            if(!$("#contractAll").val()){
                $.messager.alert('提示','请录入土地合同编号','info');
                return;
            }
...
//            console.debug(data);
            ...
            setTimeout(function(){
                ...
                if(data.ffileno){
                    var spanHtml = "";
                    $.each((data.ffileno).substring(0,$("#estateAll").val().length-1).split(','), function(key, value){
                        spanHtml += '<span data-no="'+ value +'">'+ value +'<b class="del">x</b></span>';
                    });
                    $("#estateList").html(spanHtml);
                    $("#addEstateNo").val("");
                ...
                if(data.fctrno_new){
                    var spanHtml = "";
                    $.each((data.fctrno_new).substring(0,$("#contractAll").val().length-1).split(','), function(key, value){
                        spanHtml += '<span data-no="'+ value +'">'+ value +'<b class="del">x</b></span>';
                    });
                    $("#contractList").html(spanHtml);
                    $("#addContractNo").val("");
