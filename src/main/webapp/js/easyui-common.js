...
$(function(){
	...
	*/
    ...
    $(".tabs-chart ul").on("click", "li", function(){
        var _this = $(this);
        _this.parent().find("li").removeClass("on")
        var target = _this.data("target");
        var grid = _this.data("grid");
        _this.addClass("on");
        _this.parent().parent().find("div.tabs-area").css("display","none");
        $("#" + target).css("display","block");
        var flag = _this.parent().parent().parent().parent().find(".grid-box");
        if(flag.html() != undefined){
            $(".grid-box").css("display","none");
        }
        $("#" + grid).css("display","block");
        $.parser.parse("#ban");
		$.parser.parse("#huan");
        if(!$("#" + grid).data('isparse')) {
            $.parser.parse("#" + grid);
            $("#" + grid).data('isparse', 1);
        }
    });
    ...
    $(".allChecked").on("click",function(){
        var $this = $(this).parent().parent().find(".table-label");
    ...
    $("#projectView ul.list-box").on("click","li",function(){
        var $this = $(this);
        if($this.attr("data-target")) {
            var target = $this.data("target");
            $("#projectView ul.list-box li").removeClass("on");
            $this.addClass("on");
            $(".table-box>div").hide();
            $("#" + target).show();

            if(!$("#" + target).data('isparse')) {
                $.parser.parse("#" + target);
                $("#" + target).data('isparse', 1);
            }
        }
    });

    $(".but-up").on("click",function(){
        $(this).parent().next("div.content").slideToggle("fast");
    });
...
/**
 * loading方法，替换之前使用的progress()
 * start
 */
function ajaxLoading(){
    $("<div class=\"datagrid-mask\" id=\"modbg\"></div>").css({display:"block",zIndex:"99999",width:"100%",height:$(window).height()}).appendTo("#uFRList");
    $("<div class=\"datagrid-mask-msg\" id=\"modbgMsg\"></div>").html("正在处理，请稍候。。。").appendTo("#uFRList").css({display:"block",zIndex:"99999",left:($("#uFRList").outerWidth(true) - 190) / 2,top:($(window).height() - 45) / 2});
...
function doReset() {
	...
	$(".searchForm select").each(function(){
		var resetSe = $(this).children("option:first-child").text();
        //console.log(resetSe);
...
/*金融数据分析平台form表单提交*/
function search(formName,gridId){
    if(!$('#'+formName).form('validate')) {
        return ;
    }
    var paramObj = {};
    $.each($('#'+formName).serializeArray(),function(i,data){
        paramObj[data.name] = data.value;
    });
    $("#"+gridId).datagrid("load",paramObj);
}
/*下拉框和输入框的组合，修改input name*/
function changeName3(value){
    $('#textId').attr('name',value);
}
/*提示选中数据*/
...
/*提示取消关注失败状态*/
function messageUnAttSignFalse(){
	$.messager.show({
		title:GV.MESSAGER_TITLE,
		msg:'取消关注失败，请重试！',
		timeout:1500
...
/*银行资产管理通用的combobox*/
//区域+开发商
function createCombobox(data1){
    $("#cc2").combobox({
        valueField:'value',
        textField:'text',
        editable:false,
            panelHeight:'auto',width:130,height:30,
        data:data1,
        onSelect: function(rec){
            changeName3(rec.value);
        }
    });
}

//按地区联动
function createCombobox1(data3,width){
    ...
    $('#cc1').combobox({
        ...
        onSelect: function(rec){
            changeName(rec.value);
            if(rec.text == "按支行"){
                $("#regionCombobox").css("display","none");
                $("#debteeCodeCombobox input").attr("disabled",false);
                $("#regionCombobox input").attr("disabled",true);
                $("#debteeCodeCombobox").css("display","inline");
            }else if(rec.text == "按地区"){
                $("#debteeCodeCombobox").css("display","none");
                $("#regionCombobox input").attr("disabled",false);
                $("#debteeCodeCombobox input").attr("disabled",true);
                $("#regionCombobox").css("display","inline");
            }
        },
        onLoadSuccess:function(){ changeName($(this).combobox('getValue'))}
    });
    if(null!=data3){
        $("#cc3").combobox({
            panelHeight:'auto',
            ...
            editable:false,
            data:data3,
            onSelect: function(rec){changeName3(rec.value)}
        });
    }
    /*修改第二个下拉框的name值*/
    function changeName(value){
        $('#ccid').attr('name',value);
    }
    /*修改输入框框的name值*/
    function changeName1(value){
        $('#ccid').attr('value',value);
    }
}
/*普通combobox*/
function createCombobox2(id,data,width){
    if(null==width || width==0){
        width = 150;
    }
    $("#"+id).combobox({
        ...
        textField:'text',
        editable:false,
        panelHeight:'auto',width:width,height:30,
        data:data
    });
}
/*查询房屋详情*/
function checkHouseDetail(grid,type){
    var selected = $('#'+grid).datagrid('getSelected');
    if(null!=selected){
        if(type==undefined){
            type=null;
        }
        openDialog("title: '房屋详情',width: 890,height: 530,isIframe:false,closed: false,cache: false,url: path+'/BankCommonController/houseDetail?id="+ selected.id +"&type=" + type+"',modal: true,buttons:false");
    }else{
        messageShow();
    }
}

function topJump(projectCode,projectName,url){
    window.location.href=path+"/ProjectDetailController/init?projectCode="+projectCode+"&url="+url+"&projectName="+projectName;
}
...
/*高级查询用的combobox*/
//区域
function createHigh_c1(value){
    $('#high_c1').combobox({
        editable:false,
        valueField:'value',
        textField:'text',
        url:path+'/BankCommonController/createComBox?value='+value
    });
}
//支行
function createHigh_c2() {
    $('#high_c2').combobox({
        editable: false,
        //panelHeight: 'auto',
        height:30,
        valueField: 'value',
        textField: 'text',
        url: path+'/BankCommonController/createComBox?value=debteeCode'
    });
}
...
/*------------------------------------------楼盘----------------------------------------------------------*/
/*生成tbody下面的tr*/
function getTr(fRList){
    var houseList = fRList.houseList;
    var html = '<tr><td class="first-td">'+fRList.fno+'</td><td><ul class="rno-box">';
    for (var i in houseList){
        html+='<li class="'+houseList[i].color+'"><h4>'+houseList[i].rno+','+houseList[i].area+'</h4><h4>'+houseList[i].planInfo+'</h4><span class="' + houseList[i].index+'">' + houseList[i].indexName+'</span></li>'
    }
    html += '</ul></td></tr>';
    return html;
}
...
function getTable(data){
    var uFRList =  data.uFRList[0];
    var fr = uFRList.fRList;
    var table = '<table cellpadding="0" cellspacing="0" width="100%" class="rno-list"><thead> <tr> <td class="first-td" width="45">' + uFRList.uno+'</td> <td></td> </tr> </thead><tbody id="fnoList">';
    for(var i in fr){
        table += getTr(fr[i]);
    }
    table += '</tbody></table>';
...
/*生成顶部楼层列表*/
function getFList(fset){
    var html = '<li class="on">全部</li>';
    for(var i in fset){
        html+='<li class="fno" data-target="' +fset[i]+'">' + fset[i]+'</li>';
    }
    return html;
}
/*请求后台获取楼盘表信息*/
function getBuildingList(params,type){
    $.ajax({
        type: "post",
        url: path+"/ProjectDetailController/queryBuilding",
        data: params,
        //async: false,
        dataType: "json",
...
function unoBuilding(data){
    //楼盘表
    document.getElementById('uFRList').innerHTML=getTable(data);
    //顶部楼层列表
    document.getElementById('fList').innerHTML=getFList(data.fset);
}
/*点击楼层记载楼盘数据*/
function fnoBuilding(data){
    document.getElementById('fnoList').innerHTML=getTr(data.uFRList[0].fRList[0]);
}
//});
